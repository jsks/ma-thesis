...
the executive thus serving as a deterrent to the forms of
...
significant interaction term with public spending. In both cases an
ordinal measure of executive constraints from Polity IV
[@Marshall2019] was used. This leaves open both further work on
additional theoretical mechanisms and analyses using other datasets.
...
onset. That this may not be the case is either because executive
constraints are indeterminate to the risk of civil conflict or that
there is actually a positive relationship between the two. In the case
of the latter, it could be possible that constrained executives are
less able to effectively deal with violent challenges because of risk
aversion to violence and/or from the slowdown from having to seek
approval from various stakeholders before committing to decisive
action. Given the theoretical pathways listed above, however, there is
nonetheless a justified prior belief in favor of there existing a
...
p4 <- prior_plot(c(0, 2), dweibull, args = list(shape = 5, scale = 1))
p5 <- prior_plot(c(0, 10), dhcauchy, args = list(sigma = 1))
p6 <- prior_plot(c(0, 5), dgamma, args = list(shape = 5, rate = 5))
...
```{r warning = F}
pars.df <- readRDS("posteriors/sim/parameters.rds")
...
```{r warning = F}
latent <- readRDS("posteriors/sim/theta.rds") %>%
    arrange(true_value) %>%
    mutate(idx = 1:n())
...
```{r results = 'hide'}
x <- with(latent, mean(true_value <= codehigh & true_value >= codelow))
```
...
## Appendix C - Convergence Statistics {#appendix-c}
