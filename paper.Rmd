...
explanations centered on economic factors [@Collier2012; @Collier2009]
...
[^wbgi]: Furthermore, a common issue with many measures of
    ...
    until 1996 (TODO: cite). Given that measures of institutional
...
GDP per capita. Specifically, I use the natural log of the $RGDP^o$
variable, constant price real GDP per capita, from Penn World Tables
(PWT) v9.1 (1950-2017) to allow for both cross-country and
cross-temporal comparisons of GDP per capita [@Feenstra2015]. The
justification for taking the natural log is that the marginal effect
of the variable should decrease with higher values. In other words,
the difference between the effects of relatively high levels of GDP
per capita should be significantly smaller than the difference between
the effects of low levels of GDP per capita. Finally, I also include
an interaction term between GDP per capita and the latent measure for
executive constraints to capture the potential conditional
relationship.
...
transform mean elevation and population by the natural log with the
expectation that the scale increase should diminish as values get
larger. Finally, I include measures of ethnic fractionalization in the
form of the number of political relevant ethnic groups and the percent
of the population facing discrimination. Where there are significant
ethnic cleavages and state sponsored discrimination, ethnic ties can
