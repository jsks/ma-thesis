...
the estimated posterior mean and the observed mean.
...
countries and scaled from approximately $-2$ to $2$. Since the
complete time series for every single country can not be easily
visualised, an exploratory overview on the performance of the
executive constraints measure is provided in the form of several
different plots.  The first, figure \ref{fig:theta}, is a restricted
time series plot consisting of only China, Brazil, Nigeria, Norway,
and Russia. The countries were chosen to represent a diverse set of
geographic regions as well as political contexts. From the plot,
China, being a single party authoritarian state, is consistently
ranked the worst in executive constraints. The decline for Russia
largely follows the ascent of Vladimir Putin to the presidency at the
end of 1999 and his gradual consolidation of power in the decades that
followed.[^russia] For both Nigeria and Brazil, the measure captures
periods of military dictatorship such as the era of military rule in
Brazil following the 1964 coup d'etat and the period following Johnson
Aguiyi-Ironsi's seizure of power in the 1966 coup d'etat in Nigeria.
The last country, Norway, is an example of a consolidated polyarchy
for the time period under consideration with consistently strong
constraints against the executive office.
...
This means that for the $\beta$ parameter for executive constraints
there is a `r as.numeric(beta_s[1, 2]) * 100` percent posterior
probability that the parameter is less than zero and a 80 percent
posterior probability that an increase of one standard deviation
decreases the log-odds of conflict onset between $`r beta_s[1, 1]`$. A
one standard deviation increase in this context corresponds roughly to
the difference between the level of executive constraints for Zambia
in 1967 under the administration of Kenneth Kaunda right before the
banning of opposition parties following the 1968 election, with score
of approximately 0, and Greece in 1976 shortly after the end of
military rule and establishment of a democratic republic, with a score
of approximately 1.
...
elevation roughly matches the difference between Russia, $`r russia`$
meters, and Georgia, $`r georgia`$ meters. And, a one unit increase
for proportion discrimination is roughly equivalent to Thailand from
2002 until present with $`r thai`$ percent and Bhutan from 1989 with
$`r bhutan`$ percent.
...
factions with divergent interests active; however, their political
...
state, but that emergent may not necessarily be coded as a new civil
conflict. That said, it should be mentioned that the there is a much
higher degree of uncertainty in the estimate with the 80 percent
posterior credible interval ranging from -0.558 to 0.064.
