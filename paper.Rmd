...
[^model]: The model code is written in Stan [@Carpenter2017] and was
    ...
    $\hat{R}$ and effective sample size (ESS) [@Vehtari2019] as well as
    standard Stan diagnostics. The full code is available in
...
coefficients, I select the following weakly informative priors:
\begin{align*}
...
of civil conflict may increase. Finally, I include measures of ethnic
fractionalization in the form of the number of political relevant
ethnic groups and the percent of the population facing
discrimination. Where there are significant ethnic cleavages and state
...
[^peace_yrs]: Regarding the peace years, the variable is calculated as
...
The analysis of the results is split into two sections. First, the
...
logistic regression as well as a discussion on the overall model fit
to the outcome variable, conflict onset.
...
p1 <- ggplot(nonlg_est, aes(se, interval)) + geom_point(col = "#56B4E9", alpha = 0.2, shape = 1)
p2 <- ggplot(nonlg_est, aes(se, diff)) + geom_point(col = "#56B4E9", alpha = 0.2)
...
```{r fig.width = 7, fig.height = 6, fig.cap = "\\label{fig:theta} Theta"}
...
strictly be judged by overlapping credible intervals (TODO: Schenker
and Gentleman). For example, Romania, despite having overlapping
...
$`r chile_p`\%$ posterior probability that the estimate for 2017 is higher
than 2007.
...
probs <- apply(beta, 2, function(x) list(below = mean(x < 0), above = mean(x > 0)))

beta_s <- post_summarise(beta, probs = c(0.1, 0.9)) %>%
    mutate(Estimate = sprintf("[%.3f, %.3f]", signif(`10%`, 3), signif(`90%`, 3))) %>%
    select(-`10%`, -`90%`, -parameter)

rownames(beta_s) <- colnames(beta)

kable(beta_s, "latex", align = "c", booktabs = T, linesep = "", escape = T,
...
for (y in state) {
    print(y)
    p <- boot::inv.logit(sapply(theta_seq, f, y)) * 100

    m <- apply(p, 2, median)
    pi <- apply(p, 2, quantile, probs = c(0.2, 0.8)) %>% t

    plot(theta_seq, m, type = "l", ylim = c(min(pi[, 1]), max(pi[, 2])),
         xlab = "Executive Constraints", ylab = "Predicted Probability")
    lines(theta_seq, pi[, 1], lty = 2)
    lines(theta_seq, pi[, 2], lty = 2)
}
```


```{r eval = F}
input.df <- readRDS("posteriors/model/final.rds")
p_hat <- readRDS("posteriors/model/summary/p_hat.rds")
